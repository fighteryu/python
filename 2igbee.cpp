...
uchar  SendDat[]={0xEE,0XB1,0X10,0X00,0X0A,0X00,0X0A,0X00,0X00,0X2E,0X00,0XFF,0XFC,0XFF,0xff};
...
main()
{
	...
	UART2_Init(0XD9,0,1);//9600 1T模式 不倍频UART1_Init(0XD9,0,1);//9600 1T模式 不倍频
//	Init_ST7920();		  //液晶初始化
	 ...
	 while(1)
	 {
		 ...
		 if(flag)
		 {
		    ...
		    if(receive_dat[0]==0x00)
		     {
			    ...
			    ConvertTemp1=receive_dat[3]*256+receive_dat[4];
			    
			    ConvertHumi1=receive_dat[5]*256+receive_dat[6];
		   ...
		   else if(receive_dat[0]==0x01)
		   {
				...
				ConvertTemp2=((receive_dat[3]*256+receive_dat[4])+ConvertTemp1)*2;
				long_GPRSDAT[0]=0x00;
				long_GPRSDAT[1]=0x00;
				long_GPRSDAT[2]=0x00;
		 ...
		 if(flag1)
		   {
			  ...
			  if(receive_dat1[0]==0x00)
				...
				else if((receive_dat1[0]==0xEE)&&(receive_dat1[4]==0x0B)&&(receive_dat1[6]==0x19))
			    { 
